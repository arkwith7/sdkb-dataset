...
> `CLAUDE.md` §0, so they use that repository's shorthand:
>
> | 표기 | 뜻 |
> |---|---|
...
- SDKB v1.1 enrichment layer in ontology/sdkb-core.ttl: 6 new classes (`Semiconductor`, `Intrinsic/ExtrinsicSemiconductor`, `Dopant`, `Acceptor`, `Donor`) and 4 new ObjectProperties (`hasNextStep`, `hasSubStep` transitive, `hasAcceptor`, `hasDonor`) — all with `skos:exactMatch` back-link to SemicONTO. OWL ontology grew from 257 → 353 triples.
